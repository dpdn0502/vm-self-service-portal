...
def create_incident_ticket(vm_name, action, user_name,
    ...
    payload = {
        ...
        'comments':    'Auto-generated by IaaS Self-Service Tool'
    }
...
def create_change_request(vm_name, action, user_name,
    ...
    payload = {
        ...
        'comments': (
            'Auto-generated by IaaS Self-Service Tool'
        )
    }
    ...
    if ticket_type == 'change_request':
        if status == 'success':
            payload = {
                ...
                'close_notes': (
                    ...
                    f"by {user_name} via IaaS Portal. "
                    f"{message}"
                )
            }
        else:
        ...
        if status == 'success':
            payload = {
                ...
                'close_code':  'Solved (Permanently)',
                'close_notes': (
                    f"Resolved by IaaS Portal. {message}"
                )
            }
        else:
            payload = {
...
def build_description(vm_name, action, user_name,
    ...
    return (
        ...
        f" UTC\n"
        f"Portal:         IaaS Self-Service Tool\n"
        f"{'─' * 40}\n"
        f"Auto-generated by IaaS Self-Service Tool\n"
...
def build_close_notes(action, status, user_name):
    ...
    return (
        f"{label} {result} via IaaS Self-Service Tool "
...
def close_change_request(snow_ticket: str, close_notes: str,
                          vm_name: str = '') -> dict:
    """
    Closes a CHG ticket by number (e.g. CHG0012345).

    Fetches the sys_id by ticket number first, then PATCHes:
      state      = 4  (Closed)
      close_code = successful
      close_notes = provided text

    Used by the decommission workflow on hard-delete completion.
    Returns {'success': bool, 'sys_id': str, 'error': str}
    """
    if not Config.SNOW_INSTANCE_URL:
        return {'success': False, 'error': 'SNOW not configured'}

    try:
        # Step 1: look up sys_id by number
        search_url = (
            f"{Config.SNOW_INSTANCE_URL}"
            f"/api/now/table/change_request"
            f"?sysparm_query=number={snow_ticket}"
            f"&sysparm_fields=sys_id,number,state"
            f"&sysparm_limit=1"
        )
        r = requests.get(
            search_url,
        ...
        if r.status_code != 200 or not r.json().get('result'):
            return {
                'success': False,
                'error':   f'CHG {snow_ticket} not found in SNOW',
            }
        sys_id = r.json()['result'][0]['sys_id']

        # Step 2: PATCH to Closed
        patch_url = (
            f"{Config.SNOW_INSTANCE_URL}"
        ...
        payload = {
            ...
            'close_notes': close_notes or (
                f'VM {vm_name} permanently decommissioned via IaaS Portal. '
                f'All resources deleted. Ticket auto-closed.'
            ),
        }
        pr = requests.patch(
            patch_url,
        ...
        if pr.status_code == 200:
            ...
            return {
                'success': False,
                'error':   f'SNOW PATCH returned {pr.status_code}',
            }
